...
\#Het programma is geschreven in Libre Office Calc (Excel), is open source en wordt aangeboden onder de FreeBSD licentie.
...
\#
\# <b>NEW in versie 4.1:</b>
\# - Tabblad **Keuze** toegevoegd, hiermee kunnen de meeste relevante parameters (prijs, besparing, zomerkoeling) van de verschillende isolatiemethoden worden vergeleken, inclusief de complete subsidie berekening. (zelfs uitgebreider dan Smart-Twin)
\# - Kamertemperatuur kan nu op het hoofdblad worden gewijzigd en heeft direct invloed op de resultaten
\# - Minimale (optimale) Ventilatie verlies is verdubbeld (blijkt in de praktijk moeilijk haalbaar)
\# - Autoschaling werkte niet op grafiek, bug fixed
\#- Ontwikkeling vind vanaf nu in Libre Office plaats
\# In cel AB18 kun je een andere kamertemperatuur instellen
...
\#Bespreking van de resultaten op het basis-blad
  ...
  Keuze_en_Prioriteit | <b>NEW</b> Keuze
  Keuze_DakIsolatie
  Keuze_Totaal
\# Hiermee kun je verschillende isolatiemethoden vergelijken op alle relevante parameters.
\# Bevat ook subsidieberekening.
\# Rd, Milieu-Impact, Prijs, Koelvertraging Zomer

  Auto Schaling  |   Auto Schaling
